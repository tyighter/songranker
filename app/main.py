...
POPULARITY_RATING_COUNT_CAP = 1_000_000
...
def _song_selection_weight(song: Song, popularity_weight: float) -> float:
    ...
    count_component = 0.0
    if rating_count is not None and POPULARITY_RATING_COUNT_CAP > 0:
        min_count = min(max(rating_count, 0), POPULARITY_RATING_COUNT_CAP)
        count_component = math.log1p(min_count) / math.log1p(POPULARITY_RATING_COUNT_CAP)
